...
output <- readRDS("J13108_output.rds")
md <- read.xlsx("metadata_wPubID.xlsx")
#saveRDS(output,"output_Nov.rds")
#output_old <- readRDS("output113108_wPubidFcs.rds")
#output$sample_ids <- output_old$sample_ids
clusterlevels<-c("Th",
                 "ThN",
                 "Th1",
                 "ThCM",
...
plot_clustering_heatmap_wrapper2 <- function(fcs, cell_clustering, nclusters=40,
  ...
  #get expression
  expr <- fsApply(fcs, Biobase::exprs);expr <- expr[,subtype_markers]
...
ggdf$timepoint <- factor(output$meta_data$timepoint[match(ggdf$sample_id,output$meta_data$sample_ids)], levels = timepointlevels)
ggdf$pubID <- factor(output$meta_data$Pub_ID[match(ggdf$sample_id,output$meta_data$sample_ids)], levels = ptlevels)
ggdf$study <- "J13108"

output_j14113 <- readRDS("E:/WJHLab/cytofAtlas/updated_data/J14113_PBMC/Tcell_stim/Tcell_stim/results_October/output14113TS_wPubID.rds")
...
ggdfb$timepoint <- factor(output_j14113$meta_data$timepoint[match(ggdfb$sample_id,output_j14113$meta_data$sample_ids)], levels = c('C1D1','C3D1','C4D1'))
ggdfb$pubID <- factor(output_j14113$meta_data$pubID[match(ggdfb$sample_id,output_j14113$meta_data$sample_ids)], levels = unique(output_j14113$meta_data$pubID))
ggdfb$condition <- factor(output_j14113$meta_data$condition[match(ggdfb$sample_id,output_j14113$meta_data$sample_ids)], levels = c('A','B'))
ggdfb$study <- "J14113"
...
exprmntbl$pubID <- factor(output$meta_data$Pub_ID[match(exprmntbl$sample_id,output$meta_data$sample_ids)], levels = ptlevels)
...
exprmntblb <- data.frame(fsApply(output_j14113$fcs,exprs)[, c(output_j14113$functional_markers)], #need to use the same markers as J13108 so that statistical tests will run properly
                        sample_id = output_j14113$sample_ids, 
                        cluster = output_j14113$cell_clustering1m) %>%
  group_by(sample_id, cluster) %>%
  summarize_all(funs(mean))
clusterlevels2 <- unique(exprmntblb$cluster)
exprmntblb$cluster <- factor(exprmntblb$cluster, levels=clusterlevels2)
exprmntblb$pubID <- factor(output_j14113$meta_data$pubID[match(exprmntblb$sample_id,output_j14113$meta_data$sample_ids)], levels = unique(output_j14113$meta_data$pubID))
exprmntblb$timepoint <- factor(output_j14113$meta_data$timepoint[match(exprmntblb$sample_id,output_j14113$meta_data$sample_ids)], levels = c('C1D1','C3D1','C4D1'))
exprmntblb$condition <- factor(output_j14113$meta_data$condition[match(exprmntblb$sample_id,output_j14113$meta_data$sample_ids)], levels = c('A','B'))
exprmntblb <- exprmntblb[,c("sample_id",
                            "cluster",
                            "OX40",
                            "TNFa",
                            "TIM3",
...
#colnames(ggdf2_wide2b)[2] <- "Pub_ID"
ggdf_widecombined2 <- rbind(ggdf2_wide2b,ggdf2_wide2)
