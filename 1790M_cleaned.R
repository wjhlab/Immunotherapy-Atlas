...
workd<-getwd()
output <- readRDS(paste0(workd,"/output1790M_wPubid.rds"))
#remove patient info -> replace it with pub ID
metadata <- read_xlsx(paste0(workd,"/J1790_metadataM_wPubID.xlsx"))

#set levels
clusterMergeFile <- paste0(workd,'/J1790_mergingM.xlsx')

clusterlevels = clusterlevels = c("B",
                                  "Baso",
                                  "DC_I",
                                  "DC_II",
                                  "Mono_I",
...
ptlevels <- unique(output$meta_data$pubID)
...
#saveRDS(output,paste0(workd,"/output_wPubid.rds"))
...
plot_clustering_heatmap_wrapper2(fcs=output$fcs,
                                 ...
                                 subtype_markers=output$subtype_markers,
                                 fileName = paste0(workd,'/clusteringheatmap_merged_new.pdf'))
...
#for T cell oriented
pdf(paste0(workd,"/abundanceLine_col_tp_AvB.pdf"), width=7, height=8.5);lineplot_r_A; lineplot_r_B;dev.off()
pdf(paste0(workd,"/abundanceLine_col_tp_AvB_wstats.pdf"), width=7, height=8.5)
lineplot_r_A+stat_compare_means(paired=T, size=1)
lineplot_r_B+stat_compare_means(paired=T, size=1)
dev.off()

#for myeloid oriented
pdf(paste0(workd,"/abundanceLine_col_tp_AvB.pdf"), width=7, height=5);lineplot_r_A; lineplot_r_B;dev.off()
pdf(paste0(workd,"/abundanceLine_col_tp_AvB_wstats.pdf"), width=7, height=5)
lineplot_r_A+stat_compare_means(paired=T, size=1)
lineplot_r_B+stat_compare_means(paired=T, size=1)
dev.off()
...
#for lymphoid
pdf(paste0(workd,"/abundanceLine_col_tp_both.pdf"), width=7, height=8.5);lineplot_r_all;dev.off()
pdf(paste0(workd,"/abundanceLine_col_tp_both_wstats.pdf"), width=7, height=8.5)
lineplot_r_all+stat_compare_means(paired=T, size=1)
dev.off()

#for myeloid
pdf(paste0(workd,"/abundanceLine_col_tp_both.pdf"), width=7, height=5);lineplot_r_all;dev.off()
pdf(paste0(workd,"/abundanceLine_col_tp_both_wstats.pdf"), width=7, height=5)
lineplot_r_all+stat_compare_means(paired=T, size=1)
dev.off()
...
#for T cell oriented
pdf(paste0(workd,'/abundanceFCbars.pdf'),width=7,height=7);ggfcA;ggfcB;dev.off()

#for myeloid oriented
pdf(paste0(workd,'/abundanceFCbars.pdf'),width=7,height=5);ggfcA;ggfcB;dev.off()
...
write.csv(ggdf_wide2_p[,c('cluster','p_value','diffAB')],paste0(workd,'/abundanceFC_AvB.csv'))
...
exprmntbl$pubID <- factor(output$meta_data$pubID[match(exprmntbl$sample_id,output$meta_data$sample_ids)], levels = ptlevels)
exprmntbl$timepoint <- factor(output$meta_data$timepoint[match(exprmntbl$sample_id,output$meta_data$sample_ids)], levels = timepointlevels)
exprmntbl$condition <- factor(output$meta_data$condition[match(exprmntbl$sample_id,output$meta_data$sample_ids)], levels = conditionlevels)
...
#for myeloid
pdf(paste0(workd,'/functionalLineplots_A.pdf'),width=8.5,height=5)
...
#for myeloid
pdf(paste0(workd,'/functionalLineplots_B.pdf'),width=8.5,height=5)
...
#for myeloid
pdf(paste0(workd,'/functionalLineplots_all.pdf'),width=8.5,height=5)
...
#for myeloid
pdf(paste0(workd,'/functionalFCplots_AvB.pdf'),width=7,height=5)
...
write.csv(ggdf2_wide2_p[,c('cluster','variable','p_value','diffAB')],paste0(workd,'/functionalFC_AvB.csv'))
