...
#read and cluster
workd<-getwd()
output <- readRDS("output1790TS_wPubid.rds")
#remove patient info -> replace it with pub ID
metadata <- read_xlsx(paste0(workd,"/J1790_metadataTS_wPubID.xlsx"))
...
ggdf$timepoint <- factor(output$meta_data$timepoint[match(ggdf$sample_ids,output$meta_data$sample_idss)], levels = timepointlevels)
ggdf$pubID <- factor(output$meta_data$pubID[match(ggdf$sample_ids,output$meta_data$sample_idss)], levels = ptlevels)
ggdf$condition <- factor(output$meta_data$condition[match(ggdf$sample_ids,output$meta_data$sample_idss)], levels = conditionlevels)
