...
#read and cluster
setwd("E:/WJHLab/cytofAtlas/updated_data/J1790_PBMC/Tcell_stim/Tcell_stim")

workd<-getwd()
output_wjh <- readRDS(paste0(workd,"/finalrun_revised","/backup_output_WH.rds"))
#remove patient info -> replace it with pub ID
pubID <- read_xlsx("E:/WJHLab/cytofAtlas/updated_data/J1790_PBMC/J1790 Master File 2022-11-15 DRAFT WH.xlsx")
metadata <- read_xlsx(paste0(workd,"/Config","/J1790_metadataL_added.xlsx"))

# Extract last three digits of pubID in df2
pubID$last_three_digits <- substr(pubID$`subject ID`, start = 4, stop = 6)
...
colnames(metadata)[colnames(metadata) == "sample_id"] <- "sample_id_old"
metadata$sample_id <- paste0(metadata$pubID[],"_",metadata$timepoint)
metadata$sample_id <- paste0(substr(metadata$sample_id, 1, 1), substr(metadata$sample_id, 3, nchar(metadata$sample_id)))
metadata <- metadata[,!names(metadata) %in% c("patient_id","last_three_digits")]

indices <- match(output_wjh$sample_ids,metadata$sample_id_old)
output_wjh$sample_ids <- metadata$sample_id[indices]

metadata <- metadata[,!names(metadata) %in% "sample_id_old"]
names(metadata)[8] <- "sample_ids"
output_wjh$meta_data <- metadata
output <- output_wjh
#set levels
clusterMergeFile = paste0(workd,"/Config",'/J1790_merging.xlsx')
...
samplevels <- metadata$sample_ids
...
#saveRDS(output,paste0(workd,"/finalrun_revised/output_wPubid.rds"))
...
plot_clustering_heatmap_wrapper2(fcs=output$fcs,
                                 ...
                                 subtype_markers=output$subtype_markers,
                                 fileName = paste0(workd,'/finalrun_revised/clusteringheatmap_merged_new.pdf'))
dev.off()

saveRDS(output,paste0(workd,'/finalrun_revised/outputTcellStim_carter_fixed.rds'))
...
ggdf$sample_ids <- factor(ggdf$sample_ids)
ggdf$cluster <- factor(ggdf$cluster, levels=clusterlevels)
ggdf$timepoint <- factor(output$meta_data$timepoint[match(ggdf$sample_ids,output$meta_data$sample_ids)], levels = timepointlevels)
ggdf$pubID <- factor(output$meta_data$pubID[match(ggdf$sample_ids,output$meta_data$sample_ids)], levels = ptlevels)
ggdf$condition <- factor(output$meta_data$condition[match(ggdf$sample_ids,output$meta_data$sample_ids)], levels = conditionlevels)
...
#for T cell oriented
pdf(paste0(workd,"/finalrun_revised/abundanceLine_col_tp_AvB.pdf"), width=7, height=8.5);lineplot_r_A; lineplot_r_B;dev.off()
pdf(paste0(workd,"/finalrun_revised/abundanceLine_col_tp_AvB_wstats.pdf"), width=7, height=8.5)
lineplot_r_A+stat_compare_means(paired=T, size=1)
lineplot_r_B+stat_compare_means(paired=T, size=1)
dev.off()

#for myeloid oriented
pdf(paste0(workd,"/finalrun_revised_revised/abundanceLine_col_tp_AvB.pdf"), width=7, height=5);lineplot_r_A; lineplot_r_B;dev.off()
pdf(paste0(workd,"/finalrun_revised_revised/abundanceLine_col_tp_AvB_wstats.pdf"), width=7, height=5)
lineplot_r_A+stat_compare_means(paired=T, size=1)
lineplot_r_B+stat_compare_means(paired=T, size=1)
dev.off()
...
#for lymphoid
pdf(paste0(workd,"/finalrun_revised/abundanceLine_col_tp_both.pdf"), width=7, height=8.5);lineplot_r_all;dev.off()
pdf(paste0(workd,"/finalrun_revised/abundanceLine_col_tp_both_wstats.pdf"), width=7, height=8.5)
lineplot_r_all+stat_compare_means(paired=T, size=1)
dev.off()

#for myeloid
pdf("abundanceLine_col_tp_both.pdf", width=7, height=5);lineplot_r_all;dev.off()
pdf("abundanceLine_col_tp_both_wstats.pdf", width=7, height=5)
lineplot_r_all+stat_compare_means(paired=T, size=1)
dev.off()
...
#for T cell oriented
pdf(paste0(workd,'/finalrun_revised/abundanceFCbars.pdf'),width=7,height=7);ggfcA;ggfcB;dev.off()

#for myeloid oriented
pdf('abundanceFCbars.pdf',width=7,height=5);ggfcA;ggfcB;dev.off()
...
write.csv(ggdf_wide2_p[,c('cluster','p_value','diffAB')],paste0(workd,'/finalrun_revised/abundanceFC_AvB.csv'))
...
#for lymphoid
pdf(paste0(workd,'/finalrun_revised/functionalLineplots_A.pdf'),width=8.5,height=8.5)
#for myeloid
pdf('functionalLineplots_A.pdf',width=8.5,height=5)
...
#for lymphoid
pdf(paste0(workd,'/finalrun_revised/functionalLineplots_B.pdf'),width=8.5,height=8.5)
#for myeloid
pdf('functionalLineplots_B.pdf',width=8.5,height=5)
...
#for lymphoid
pdf(paste0(workd,'/finalrun_revised/functionalLineplots_all.pdf'),width=8.5,height=8.5)
#for myeloid
pdf('functionalLineplots_all.pdf',width=8.5,height=5)
...
#for lymphoid
pdf(paste0(workd,'/finalrun_revised/functionalFCplots_AvB.pdf'),width=7,height=7)
#for myeloid
pdf('functionalFCplots_AvB.pdf',width=7,height=5)
...
write.csv(ggdf2_wide2_p[,c('cluster','variable','p_value','diffAB')],paste0(workd,'/finalrun_revised/functionalFC_AvB.csv'))
